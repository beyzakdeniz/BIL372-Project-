...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["inputValue"])) {
        $selectedDers = $_POST["inputValue"];

        echo "<form action=ders_ekle_back.php method=POST>
        <h1>Yeni Ders Bilgileri</h1>
        <label for=dersAdi>
        <div id=DersAdi>
            <label for= selectedDers id= selectedDers name=selectedDers >Ders Adı:</label>
            <label> $selectedDers </label>
        </div><br>
        </label>
        ";
...
<body>
        ...
        <label for="ders_adi">Ders Adi:</label>
        <textarea style="font-size: 14px; width: 150px; height: 20px;" id="ders_adi" type="text" name="ders_adi"></textarea><br>
        
        
        
        </label>
        <label for="ders_kodu">Ders Kodu:</label>
        <textarea style="font-size: 14px; width: 150px; height: 20px;" id="ders_kodu" type="text" name="ders_kodu"></textarea><br>
        
        
        <h1>Derse Öğretmen Ekle </h1>
